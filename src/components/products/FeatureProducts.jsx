...
const FeatureProducts = ({ products }) => {
    ...
    return (
        ...
        <div className='w-[100%] xs:w-full flex flex-wrap justify-center mx-auto overflow-hidden'>

            <div className='flex justify-start flex-col relative xs:text-lg text-slate-600 font-bold xs:pb-3 text-4xl pb-11'>
                <h2>FOOD PACKAGING</h2>
            </div>

            <div className='justify-center xs:gap-0 gap-5 rounded-3xl xs:w-full grid grid-cols-5 sm:grid-cols-2 xs:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 md-lg:grid-cols-2 2xl:grid-cols-4'>
                {
                    products.map((p, i) => <div key={i} className='h-[500px] box-shadow: rgba(100, 100, 111, 0.2) 0px 7px 29px 0px; xs:mb-2 w-[319px] xs:ml-1 xs:w-[180px] xs:h-[300px] group border p-2 bg-[#ffffff] rounded-3xl group transition-all duration-500 hover:shadow-md '>
                        <div className='relative overflow-hidden'>
                            <Link to={`/product/details/${p.slug}`}><img className='sm:w-full w-[298px] h-[315px]  xs:w-[164px] rounded-3xl xs:h-[175px]' src={p.images[0]} alt="product images" ></img></Link>
                        ...
                        <div className='xs:mt-[2px] px-1 mt-[19px] text-black'>
                            <span class='line-clamp-2 mt-[4px] font-lato bg-aquamarine xs:w-36 w-72'><span className='font-bold text-[15px] mr-2'>{p.name}</span>{p.description}</span>
                            
                            <div className='flex xs:mt-0 mt-[10px] xs:justify-between'>
                                <span className='mr-3 xs:mt-[-3] font-lato '>By:Shop</span>
                            ...
                            <div className='flex  '>
                                <div className='flex flex-col mr-10'>
                                    {
                                        <span className="text-black xs:text-[1.0em]  text-[1.62em]   w-[auto] font-bold">${p.price - Math.floor((p.price * p.discount) / 100)}</span>
                                ...
                                <div className='flex' >
                                    <li onClick={() => add_card(p._id)} className='w-[36px] mr-2 h-[36px]  cursor-pointer text-white bg-red-500  text-bold flex justify-center items-center rounded-full text-black transition-all xs:hidden'><AiOutlineShoppingCart /></li>

                                    <Link  onClick={handleOpen}><button className='xs:w-[55px] xs:h-[23px] xs:ml-[-22px] w-[119px] h-[40px] cursor-pointer hover:shadow-lg hover:shadow-red-500/40 bg-red-500 rounded-full text-white'>Buy <span className='xs:hidden'>Now</span></button> </Link>
